#!/usr/bin/env python3
"""
search_pdf.py: Search for terms in PDF files within a directory, supporting complex Boolean expressions (AND, OR, parentheses).

Usage:
  python search_pdf.py /path/to/folder -s "EXPR"
  python search_pdf.py /path/to/folder -f terms.txt

Each expression can be on one line; multiline expressions in a file will be joined automatically if parentheses span lines.
During execution, a progress report is printed showing:
  - Current PDF being processed
  - Percentage of files processed
  - Number of PDFs matching criteria so far
"""
import argparse
import os
import glob
...
class Parser:
    ...
    def parse_or(self) -> tree:
        ...
        while self.pos < len(self.tokens) and self.tokens[self.pos] == 'OR':
            self.pos += 1; nodes.append(self.parse_and())
    ...
    def parse_and(self) -> tree:
        ...
        while self.pos < len(self.tokens) and self.tokens[self.pos] == 'AND':
            self.pos += 1; nodes.append(self.parse_term())
...
def load_queries(search: str, search_file) -> List[str]:
    if search_file:
        ...
        queries = [search]
    if not queries:
        print('No search terms found.', file=sys.stderr); sys.exit(1)
    return queries
...
def search_in_pdf(path: str, trees: List[tree], queries: List[str]) -> List[str]:
    try:
    ...
    return [queries[i] for i, node in enumerate(trees) if node.evaluate(text)]
...
def main():
    ...
    group.add_argument('-f', '--search-file', type=argparse.FileType('r', encoding='utf-8'),
                       help='File with expressions or JSON list')
    args = parser.parse_args()

    if not os.path.isdir(args.folder):
        print(f"'{args.folder}' is not a valid directory", file=sys.stderr); sys.exit(1)
    ...
    for idx, pdf in enumerate(pdfs, start=1):
        ...
        matched = search_in_pdf(pdf, trees, queries)
        if matched:
            results[pdf] = matched
            matched_count += 1
        # progress report
        print(f"Processing: {pdf} ({idx}/{total}, {percent:.1f}% complete) - Matches so far: {matched_count}")
